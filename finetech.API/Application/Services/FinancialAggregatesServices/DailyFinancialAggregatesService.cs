...
{
    public class DailyFinancialAggregatesService(IUnitOfWork unitOfWork, 
        IFinancialIntelligenceSnapshotService financialIntelligenceSnapshotService,
        IFinancialIndicatorsService financialIndicatorsService, ILogger<DailyFinancialAggregatesService> logger) : IDailyFinancialAggregatesService
    {
        public async Task CreateDailyFinancialAggregates(DateTime date, CancellationToken cancellationToken = default)
        {
            ...
            {
                var existingSnapshot = await unitOfWork.FinancialIntelligenceSnapshots.GetByDayAsync(user.Id, date, cancellationToken);
                if (existingSnapshot != null)
                {
                    logger.LogInformation("Daily financial intelligence snapshot already exists for user {UserId} on {Date}, skipping", user.Id, date);
                    continue;
                }
                try
                {
                    var snapshotDto = await financialIntelligenceSnapshotService.ComputeDailyFinancialIntelligenceSnapshot(user.Id, date, cancellationToken);
